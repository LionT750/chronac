    ...
    <div className="flex items-center justify-between border-b border-border bg-sidebar px-4 py-3 backdrop-blur-sm">
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-2 text-foreground">
          ...
          <span className="text-sm font-semibold tracking-wide">{page === 'academic-structure' ? 'Estrutura acadêmica' : page === 'teachers' ? 'Professores' : page === 'classes' ? 'Turmas' : 'Cronograma Acadêmico'}</span>
        </div>
      </div>

      <div className="flex items-center gap-2">
        <ThemeToggle />
        {onLogout && <Button variant="outline" size="sm" onClick={onLogout} disabled={loggingOut}>
